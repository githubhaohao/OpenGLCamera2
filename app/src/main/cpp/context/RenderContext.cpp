...
jfieldID ByteFlowRenderContext::s_ContextHandle = nullptr;

ByteFlowRenderContext::ByteFlowRenderContext(int renderType) :
        m_pByteFlowRender(nullptr), m_pCurGlFilter(nullptr), m_pBeforeGlFilter(nullptr),
        m_bIsExampleMode(
                false) {
    switch (renderType) {
        case GL_RENDER_TYPE:
...
ByteFlowRenderContext::~ByteFlowRenderContext() {
    if (m_pByteFlowRender != nullptr) {
        delete m_pByteFlowRender;
        m_pByteFlowRender = nullptr;
    }
...
void ByteFlowRenderContext::StoreRenderContext(JNIEnv *env, jobject instance,
                                               ByteFlowRenderContext *pContext) {
    LOGCATE("ByteFlowRenderContext::StoreRenderContext");
    jclass cls = env->GetObjectClass(instance);
    if (cls == nullptr) {
        LOGCATE("ByteFlowRenderContext::StoreRenderContext cls == nullptr");
        return;
    }

    s_ContextHandle = env->GetFieldID(cls, "mNativeContextHandle", "J");
    if (s_ContextHandle == nullptr) {
        LOGCATE("ByteFlowRenderContext::StoreRenderContext s_ContextHandle == nullptr");
        return;
    }
...
void ByteFlowRenderContext::DeleteRenderContext(JNIEnv *env, jobject instance) {
    ...
    auto *pContext = reinterpret_cast<ByteFlowRenderContext *>(env->GetLongField(
...
void ByteFlowRenderContext::SetTransformMatrix(float translateX, float translateY, float scaleX,
...
void ByteFlowRenderContext::LoadLutImageData(int index, int format, int width, int height,
                                             uint8_t *pData) {
...
void ByteFlowRenderContext::LoadFragShaderScript(int shaderIndex, char *pShaderStr, int strLen) {
    LOGCATE("ByteFlowRenderContext::LoadFragShaderScript shaderIndex = %d, strLen = %d",
            shaderIndex, strLen);

    LOGCATE("ByteFlowRenderContext::LoadFragShaderScript pShaderStr = %s",pShaderStr);
